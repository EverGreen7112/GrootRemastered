...
public class Utilities implements SubsystemConstants {
    ...
    /**
     * Calculates the error of the chassis from the center of a cell using the {@link Groot#getNetworkTable()},
    ...
    public static double getChassisFix() {
        NetworkTable table =  Groot.getNetworkTable();
        double error;
        if (table.getEntry("isUpdate0").getBoolean(false)
           && table.getEntry("isUpdated1").getBoolean(false)) {
                double center = table.getEntry("x0").getDouble(0)
                            + table.getEntry("x1").getDouble(0)
                            / 2;
                error = 320 - center;
         }
